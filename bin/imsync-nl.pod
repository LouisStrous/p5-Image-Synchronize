...
=head1 OPTIES
  ...
  --recurse        Behandel ook submappen.  Dit is de standaard.
                   ...
                   --behandelen.
  --relativefiletime  Geef de bestandsaanpassingstijden dezelfde
                   lokale kloktijd als in de (verre) tijdzone waarin
                   de foto gemaakt werd, in plaats van hetzelfde
                   absolute tijdstip.
  --removebackups  Verwijder back-upbestanden.  Kan ook worden
                   geschreven als --remove-backups.
  --removeourtags  Verwijder metadata die door dit programma was
...
Een bestand voldoet als het aanmaaktijdstip uit de metadata in het
bestand (sleutelwoord I<CreateDate>) overeenkomt met het gegeven
tijdstip.
...
Een bestand voldoet als het aanmaaktijdstip uit de metadata in het
bestand in de aangegeven tijdperiode valt.
...
bevat met metadata met daarin een aanmaaktijdstip.
...
de standaardcamera-ID die van diverse metadatawaarden afhangen.
...
camera-ID aan de metadata toegevoegd (ons eigen sleutelwoord
I<XMP:CameraID>).
...
Kiest de snelheid waarmee metadata uit bestanden gelezen wordt, van 0
tot 2.  Een hoger getal geeft snellere verwerking maar kan belangrijke
informatie missen.  De standaardwaarde is 1.  Komt overeen met de
FastScan-optie in L<Image::ExifTool/Options>.
...
de GPS-positie in de metadata van het bestand heeft aanpassing nodig,
en de I<XMP:TimeSource>-sleutel is aanwezig en zijn waarde is anders
dan C<GPS> (dus de bestaande GPS-positie werd niet vastgelegd door een
GPS-ontvanger verbonden met de camera).
...
metadata uit de volgende group aanpassing nodig heeft:
...
I<XMP:TimeSource>-sleutel ontbreekt in de metadata of een waarde heeft
gelijk aan C<GPS>.  Dit betekent dat de originele GPS-positie die door
de GPS-ontvanger van de camera werd opgeslagen verloren gaat.
...
=head2 --relativefiletime

Zegt dat de lokale kloktijd van de afbeeldingen ten opzichte van de
tijdzone waarin de foto's werden gemaakt moeten worden
getransplanteerd naar de tijdzone van het lokale systeem.  Dan komen
foto's die genomen werden rond het middaguur in verschillende
tijdzones allemaal terecht bij het middaguur op het lokale systeem
(als ze op bestandsveranderingtijd gesorteerd worden).
...
Verwijdert de metadata die door vorige aanroepen van dit programma
waren toegevoegd.

De volgende metadata wordt alleen verwijderd als I<XMP:TimeSource>
gezet is en een waarde ongelijk aan C<GPS> heeft: I<GPSLongitude>,
I<GPSLatitude>, I<GPSAltitude>, I<GPSDateTime>, I<GPSDateStamp>,
I<GPSTimeStamp>.

De volgende metadata wordt altijd verwijderd: I<XMP:CameraID>,
I<XMP:ImsyncVersion>, I<XMP:TimeSource>.
...
tijdstip hooguit 12 uur voor of na de I<CreateDate>-metadata van het
doelbestand is -- dus dit werkt alleen als het doelbestand die
metadata bevat.  Dit is handig als je een tijdstip nabij de
...
I<CreateDate>-metadata, dus dit werkt alleen als het doelbestand die
metadata bevat.
...
padnaam dan is de I<oorspronkelijke> C<DateTimeOriginal>-metadata (van
voor de imsync-aanroep) van dat bestand de doeltijd voor de
doelbestanden.
...
gebaseerd op informatie geleverd door de gebruiker en op metadata
gelezen uit andere foto's van dezelfde camera's.  Het kan bovendien
geografische coördinaten toevoegen aan de metadata van foto's en
films, gebaseerd op informatie uit GPX-bestanden.

Metadata in foto- en filmbestanden wordt uitgelezen, aangepast, of
toegevoegd met hulp van L<Image::ExifTool>, dus als die module de
metadata niet kan lezen of schrijven dan kan imsync dat ook niet.  De
namen van metadatawaarden die hieronder genoemd worden zijn de namen
zoals die module ze kent.
...
Belangrijke metadata wordt onthouden en bestanden die te weinig
nuttige metadata bevatten worden verder genegeerd.
...
De doeltijd voor elk bestand wordt bepaald uit metadata uit dat
...
De meeste camera's melden het merk en model in de metadata van de
afbeeldingen (bij sleutelwoorden I<Make> en I<Model>).  Sommige
...
Als de metadata van een foto geen merk/model/serinummer bevat waaruit
een voorkeurscamera-ID kan worden gemaakt dan kan die foto de
...
serienummer in de metadata zetten.  In dat geval kan de gebruiker zelf
een onderscheidende camera-ID toekennen met de L</--camera-id>-optie.

De camera-ID wordt aan de metadata van de afbeeldingen toegevoegd
(gekoppeld aan ons eigen sleutelwoord I<XMP:CameraID>) als de
afbeeldingen aangepast worden (L</--modify>), zodat je deze camera-ID
niet opnieuw hoeft op te geven als deze afbeeldingen in een latere
aanroep van dit programma opnieuw verwerkt worden.

=head2 Cameratijdverschil

We moeten het verschil weten tussen de tijdstippen die de
cameraklokken meldden (en die in de metadata van de afbeeldingen
...
metadata van de foto 17:24:33 is en de foto genomen werd op 15:23:11
UTC, dan is het cameratijdverschil +02:01:22.
...
tijdzoneinformatie in zijn metadata heeft dan neemt imsync aan dat de
tijdstippen in die metadata zijn gegeven in de tijdzone die gold voor
die datum en tijd op het computersysteem waarop imsync uitgevoerd
wordt.  Als later andere foto's voor dezelfde camera-ID worden
verwerkt dan is er al tenminste één cameratijdverschil bekend voor die
camera-ID en die kan dan opnieuw gebruikt worden.
...
toe elk maar één foto of bestand verwerkt.  Van de camera met
camera-ID C<CameraID3> zijn al meerdere afbeeldingen verwerkt, waarvan
de tijdstippen vanaf het eerstgenoemde tot net voor het tweede werden
genomen toen de tijdzone van de camera overeenkwam met UTC+4:00, en
degenen vanaf het tweede tijdstip tot en met het laatste tijdstip
werden genomen toen de tijdzone van de camera overeenkwam met
UTC+2:00.

Sinds versie 2.001 van deze applicatie worden cameratijdzones
opgeslagen met twee tijdcomponenten, bijvoorbeeld C<+0:00:15+02:00>.
De tweede component zegt welke de tijdzone van toepassing was op de
plek waar de foto werd gemaakt, en de eerste component zegt hoeveel de
cameraklok was afgedreven van die tijdzone.  In het voorbeeld werd de
foto genomen op een plek waar de tijdzone UTC+02:00 was, en de klok
van de camera liep 15 seconden voor op die tijdzone.

Oudere versies van deze applicatie sloegen cameratijdzones op met maar
één component.  De som van de twee nieuwere componenten komt overeen
met de oudere waarde.

Het gebruik van twee componenten is van belang voor de optie
L</--relativefiletime>.
...
=head3 Afbeeldingsbestanden met metadata in UTC

De tijdstippen in de metadata in de meeste afbeeldingsbestanden geven
de camerakloktijd, die meestal is afgesteld op of nabij de lokale
tijdzone.  Echter, sommige afbeeldingsbestanden (zoals
Apple-QuickTime-bestanden) hebben tijdstippen in hun metadata waarvan
de bedoeling is dat die in de UTC-tijdzone gegeven zijn, ongeacht de
lokale tijdzone.  Die tijdstippen zijn niet altijd echt in UTC, want
veel camera's weten helemaal niet in welke tijdzone hun klok is
afgesteld en weten dus ook niet wat de tijd in UTC is.  In elk geval
kunnen tijdstippen in metadata in afbeeldingen met
bedoeld-UTC-tijdstippen in een andere tijdzone zijn dan tijdstippen in
metadata in afbeeldingen met lokale-tijdzone-tijdstippen die door
...
Momenteel vallen alleen Apple-QuickTime-filmpjes in deze categorie.
Gebruik optie L</--cameraid> om een ander camera-ID toe te kennen als
dat nodig is.

=head2 GPS-bepalingsvertraging

Een camera die verbonden is met een GPS-ontvanger (zoals in moderne
mobiele telefoons) kan een GPS-tijdstip en GPS-locatie in de metadata
van de afbeeldingsbestanden schrijven.  Dat tijdstip en die locatie
zijn de meest recente die door de GPS-ontvanger bepaald werden, maar
die bepaling kan al een paar seconden oud zijn wanneer de afbeelding
wordt genomen.  Ik noem het verschil tussen de tijd van de
...
Als er al een GPS-tijdstip in de metadata zit en er is geen aanduiding
dat dat tijdstip door een eerdere aanroep van dit programma is
toegevoegd (zie L</TimeSource>) en het afgeleide cameratijdverschil
...
imsync kan uit de I<XMP:TimeSource>-metadata afleiden wat de bron van
de locatieinformatie en I<GPSDateTime> was, want imsync voegt
...
metadata toen imsync dat bestand voor het eerst verwerkte.  imsync
past het doeltijdstip en de locatie niet aan, behalve misschien als
L</--time> of L<--force --force|/--force> wordt gebruikt voor dat
bestand.
...
aanmaaktijdstip in zijn metadata heeft (sleutel I<CreateDate>), dan is
het doeltijdstip gelijk aan het aanmaaktijdstip met daarop het
cameratijdverschil toegepast.

Anders, als er een GPS-tijdstip (sleutel I<GPSDateTime>) in de
metadata is, dan wordt het doeltijdstip daarop gebaseerd (maar zie
L</GPS-bepalingsvertraging>).
...
de aanmaakdatum uit de metadata om het doeltijdstip te vinden.

Anders, als het bestand in zijn metadata een origineel tijdstip heeft
(sleutel I<DateTimeOriginal>), dan wordt dat gebruikt als het
doeltijdstip.

Anders, als het bestand in zijn metadata een aanmaaktijdstip heeft
(sleutel I<CreateDate>), dan wordt dat gebruikt als het doeltijdstip.
...
=head2 Relatieve Bestandstijden

Standaard zijn de doeltijdstippen I<absolute tijden>.  Als twee mensen
foto's maken van dezelfde gebeurtenis (bijvoorbeeld van een
maansverduistering, of van zichzelf terwijl ze met elkaar bellen) en
dan het huidige programma goed gebruiken, dan eindigen hun foto's met
praktisch dezelfde doeltijdstippen gemeten ten opzichte van UTC, zelfs
als hun foto's werden gemaakt op plekken in heel verschillende
tijdzones.  Maar zulke absolute tijdstippen zijn niet altijd het meest
handig voor het organiseren en vertonen van je foto's en filmpjes.

Stel, je reist naar een plek in een verre tijdzone.  De dag is op die
plek een paar uur verschoven ten opzichte van thuis.  Je eet je
avondeten op de daar normale tijd, je slaapt als het daar nacht is, en
je neemt zonnige foto's als het daar dag is.  Als je weer thuis bent
en dit programma gebruikt om je foto's te synchroniseren dan komen ze
terecht bij de juiste absolute tijden, maar het dagelijkse ritme dat
in die foto's te zien is is een paar uur verschoven ten opzichte van
de kloktijden van thuis.  Die foto van je diner kan terechtkomen na
middernacht in je thuistijdzone, en die zonnige foto's kunnen in je
thuistijdzone diep in de nacht terechtkomen.  Als het programma
waarmee je de foto's toont ook de laatste bestandsverandertijd laat
zien dan geven die niet de kloktijd aan die het toen was op die verre
plek maar de kloktijd die het op hetzelfde moment thuis was.

Als L</--relativefiletime> wordt gebruikt dan wordt het doeltijdstip
aangepast voor het verschil tussen de verre en thuistijdzones zodat
foto's die rond het midden van de dag op de verre plek gemaakt werden
ook op het huidige systeem terechtkomen rond het midden van de dag.

Alleen de bestandsverandertijd wordt op die manier aangepast.  Het
tijdstip dat in de foto wordt opgenomen (I<DateTimeOriginal>, en
I<GPSDateTime> als die beschikbaar is) is nog steeds de absolute tijd.
...
=head2 Metadata-bestanden

Als sommige nuttige metadata-waarden niet in het afbeeldingbestand
zitten dan kunnen ze uit een bijbehorend I<metadata-bestand> gelezen
worden.  Het metadata-bestand moet hetzelfde
L<bestandsnummer|Bestandsnummers> hebben als het afbeeldingbestand
waarop de metadata van toepassing is, en de inhoud moet JSON- of
YAML-formaat hebben.  Een JSON-bestand kan op basis van de inhoud
herkend worden, maar een YAML-bestand moet een bestandsnaamuitgang
I<.yaml> of I<.yml> hebben (in kleine of hoofdletters) om herkend te
worden.

Het JSON- of YAML-bestand moet een verzameling van sleutels en waarden
bevatten, of een lijst met daarin een enkel element die gelijk is aan
zo'n verzameling.  Sleutels die niet gelijk zijn aan een
ExifTool-sleutel die voor deze applicatie interessant is worden
genegeerd.  De sleutels hebben bij voorkeur de ExifTool-groepnaam aan
het begin.  De bijbehorende waarde is de waarde die voor het
afbeeldingbestand bij die sleutel hoort.  Voor waarden die numeriek
zijn moet die waarde een enkel getal zijn, niet geformatteerd als een
tekstwaarde die (o.a.) meerdere getallen bevat.  Bijvoorbeeld, de
waarde die behoort bij de C<EXIF:GPSLatitude>-sleutel moet er ongeveer
uitzien zoals <51.045711184> en niet zoals C<"51 deg 2' 44.56\$"">.

JSON met de juiste opmaak wordt geproduceerd door C<exiftool -G -j -n
BESTAND> waar C<BESTAND> de padnaam van het afbeeldingbestand met de
gewenste metadata erin.

Een JSON- of YAML-bestand dat er schematisch als volgt uitzien zou
...
imsync can de geografische locatieinformatie in de metadata van een
bestand aanpassen of toevoegen.

Als de locatieinformatie die al in de metadata van een
...
GPS-positie verliezen die door de originele camera in de metadata van
het bestand werd geschreven.
...
Als het bestand aangepast wordt dan worden alle veranderde metadata
bijgewerkt, maar de GPS-positie alleen als die flexibel is of als
C<--force --force> is gebruikt.  Als het bestand wordt aangepast en al
een GPS-positie had maar nog geen I<XMP:TimeSource>, dan krijgt het
een I<XMP:TimeSource> gelijk aan C<GPS>.  Als de GPS-positie nieuw is
en door L</--location> gezet werd, dan krijgt I<XMP:TimeSource> de
waarde C<User>.  Als de GPS-locatie uit een GPX-bestand kwam dan
krijgt I<XMP:TimeSource> de waarde C<Other>.
...
metadata te schrijven maakt zelf ook een back-up voordat er iets
...
Tel er 2 bij op om de originele metadata van de verwerkte bestanden te
tonen.

Tel er 4 bij op voor details van de bepaling van de uiteindelijke
metadata voor alleen de bestanden waarvoor een aanpassing aangewezen
is.

Tel er 8 bij op voor details van de bepaling van de uiteindelijke
metadata voor alle verwerkte bestanden.
...
(E) het originele tijdstip uit de metadata (I<DateTimeOriginal>), (O)
diverse metadatavelden die niet onder een van de andere kolommen
vallen, (M) het bewerkingstijdstip uit het bestandssysteem, en (P) de
GPS-locatie (I<GPSLatitude>, I<GPSLongitude> en misschien
I<GPSAltitude>).  De F-kolom toont welk L</--force>-niveau nodig is om
het bestand aangepast te krijgen (0 betekent geen C<--force>, 1
betekent C<--force>, en 2 betekent C<--force --force>).  In de andere
kolommen betekent C<-> dat de metadata afwezig blijft.  Een C<=>
betekent dat die metadata aanwezig was en ongewijzigd blijft.  Een
C<*> betekent dat die metadata wordt aangepast.  Een C<+> betekent dat
die metadata wordt toegevoegd.  Een C<!> betekent dat de metadata
wordt verwijderd.
...
I<CreateDate> uit de metadata, op vergelijkbare manier als in het "M
Offset"-veld.  Bijvoorbeeld, C<+2h> betekent dat de doeltijd precies 2
uur later is dan de I<CreateDate>.
...
=item L<Image::ExifTool> versie 10.14 of hoger
